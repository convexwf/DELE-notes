# Verb Basics

## 🔹 What is a Verb?

- A verb expresses **action**.
  - Examples: *run, sit, eat, swim, study*
- The **infinitive** is the base form of a verb:
  - English: *to run, to swim*
  - Spanish: *hablar (to speak), comer (to eat), vivir (to live)*

## 🔹 Regular Verbs

### Three Types of Spanish Verbs

All Spanish verbs are either **regular** or **irregular**. Regular verbs follow predictable patterns based on their infinitive endings:

- **-ar verbs** (like *hablar* - to speak)
- **-er verbs** (like *comer* - to eat)  
- **-ir verbs** (like *vivir* - to live)

### How to Conjugate Regular Verbs

**Step 1: Drop the Infinitive Ending**

Remove the -ar, -er, or -ir from the end of the verb.

**Step 2: Add the Appropriate Ending**

The ending depends on:

1. The **subject** (who is doing the action)
2. The **verb type** (-ar, -er, or -ir)

### Present Tense Conjugation Patterns

**-ar Verbs (hablar - to speak)**

| Subject Pronoun  | End### Practice Examples

**Statements:**

```markdown
Hay una computadora en mi cuarto.
→ There is a computer in my room.

Hay veinte estudiantes en la clase.
→ There are twenty students in the class.
```

**Questions:**

```markdown
¿Hay agua en el refrigerador?
→ Is there water in the refrigerator?

¿Hay tareas para mañana?
→ Is there homework for tomorrow?
```

**Answers:**

```markdown
Sí, hay.
→ Yes, there is/are.

No, no hay.
→ No, there isn't/aren't.
```

> **💡 Memory Tip:** Think of "hay" as the Spanish "there is/are" that never changes its form. Whether you're talking about one elephant or a thousand elephants, you always use "hay" - **Hay un elefante** (there is an elephant) and **Hay mil elefantes** (there are a thousand elephants). ---------------- | ------ | -------- |
| yo               | -o     | hablo    |
| tú               | -as    | hablas   |
| él/ella/Ud.      | -a     | habla    |
| nosotros/as      | -amos  | hablamos |
| vosotros/as      | -áis   | habláis  |
| ellos/ellas/Uds. | -an    | hablan   |

**-er Verbs (comer - to eat)**

| Subject Pronoun  | Ending | Example |
| ---------------- | ------ | ------- |
| yo               | -o     | como    |
| tú               | -es    | comes   |
| él/ella/Ud.      | -e     | come    |
| nosotros/as      | -emos  | comemos |
| vosotros/as      | -éis   | coméis  |
| ellos/ellas/Uds. | -en    | comen   |

**-ir Verbs (vivir - to live)**

| Subject Pronoun  | Ending | Example |
| ---------------- | ------ | ------- |
| yo               | -o     | vivo    |
| tú               | -es    | vives   |
| él/ella/Ud.      | -e     | vive    |
| nosotros/as      | -imos  | vivimos |
| vosotros/as      | -ís    | vivís   |
| ellos/ellas/Uds. | -en    | viven   |

### Key Patterns to Remember

**📌 yo Form**

- **All verbs** end in **-o** regardless of type
- hablar → hablo, comer → como, vivir → vivo

**📌 tú Form**  

- **-ar verbs**: add **-as** (hablas)
- **-er/-ir verbs**: add **-es** (comes, vives)

**📌 él/ella/Ud. Form**

- **-ar verbs**: add **-a** (habla)
- **-er/-ir verbs**: add **-e** (come, vive)

**📌 nosotros/as Form**

- **-ar verbs**: add **-amos** (hablamos)
- **-er verbs**: add **-emos** (comemos)
- **-ir verbs**: add **-imos** (vivimos)

**📌 vosotros/as Form (Spain only)**

- **-ar verbs**: add **-áis** (habláis)
- **-er verbs**: add **-éis** (coméis)
- **-ir verbs**: add **-ís** (vivís)

**📌 ellos/ellas/Uds. Form**

- **-ar verbs**: add **-an** (hablan)
- **-er/-ir verbs**: add **-en** (comen, viven)

### Three Meanings of Present Tense

Spanish present tense expresses **three different meanings** in English:

**Yo hablo inglés**

- I speak English
- I do speak English  
- I am speaking English

**Tú comes pan**

- You eat bread
- You do eat bread
- You are eating bread

### When to Use Subject Pronouns

**Usually Optional**

Spanish verb endings indicate who performs the action, so pronouns are often unnecessary:

- **Hablo español.** (yo is not necessary)
- **Comemos carne.** (nosotros is not necessary)  
- **Vives en México.** (tú is not necessary)

**Used for Clarification**

When the verb form is ambiguous:

- **Ud. habla** / **él habla** / **ella habla** (all use "habla")

**Used for Emphasis**

To contrast or emphasize:

- **Ella habla español y yo hablo inglés.**
- **Yo como pan y tú comes arroz.**

### Using Two Verbs Together

Like English, you can use two verbs in sequence:

- First verb: **conjugated**
- Second verb: **infinitive form**

**Examples:**

- **Necesito hablar** con Alfredo. (I need to speak with Alfredo)
- **Deseas vivir** en España. (You wish to live in Spain)

### Common Regular -ar Verbs

| Verb      | Translation                     | Verb        | Translation          |
| --------- | ------------------------------- | ----------- | -------------------- |
| alquilar  | to rent                         | entrar (en) | to enter (into)      |
| amar      | to love                         | enviar      | to send              |
| andar     | to walk                         | escuchar    | to listen to         |
| ayudar    | to help                         | esperar     | to hope, to wait for |
| bailar    | to dance                        | estudiar    | to study             |
| buscar    | to look for                     | firmar      | to sign              |
| caminar   | to walk                         | ganar       | to win, earn         |
| cantar    | to sing                         | gastar      | to spend money       |
| cocinar   | to cook                         | hablar      | to speak, to talk    |
| comprar   | to buy                          | lavar       | to wash              |
| contestar | to answer                       | llegar      | to arrive            |
| dejar     | to allow, to leave              | llevar      | to wear, to carry    |
| desear    | to desire                       | mandar      | to order             |
| enseñar   | to teach                        | mirar       | to watch, to look at |
| necesitar | to need                         | olvidar     | to forget            |
| pagar     | to pay for                      | practicar   | to practice          |
| preguntar | to ask                          | preparar    | to prepare           |
| regresar  | to return                       | saludar     | to greet             |
| tocar     | to touch, to play an instrument | tomar       | to take, to drink    |
| trabajar  | to work                         | viajar      | to travel            |
| visitar   | to visit                        |             |                      |

### Common Regular -er Verbs

| Verb       | Translation        | Verb     | Translation        |
| ---------- | ------------------ | -------- | ------------------ |
| aprender   | to learn           | creer    | to believe         |
| beber      | to drink           | deber    | to have to, to owe |
| comer      | to eat             | esconder | to hide            |
| comprender | to understand      | leer     | to read            |
| correr     | to run             | meter en | to put into        |
| poseer     | to possess, to own | prometer | to promise         |
| romper     | to break           | temer    | to fear            |
| vender     | to sell            |          |                    |

### Common Regular -ir Verbs

| Verb      | Translation        | Verb      | Translation |
| --------- | ------------------ | --------- | ----------- |
| abrir     | to open            | descubrir | to discover |
| admitir   | to admit           | discutir  | to discuss  |
| asistir a | to attend          | escribir  | to write    |
| cubrir    | to cover           | existir   | to exist    |
| decidir   | to decide          | omitir    | to omit     |
| describir | to describe        | partir    | to divide   |
| permitir  | to permit          | recibir   | to receive  |
| subir     | to climb, to go up | sufrir    | to suffer   |
| unir      | to unite           | vivir     | to live     |

> **💡 Study Tip:** Create flashcards with the verb patterns and practice conjugating different verbs using the same endings. Master the three model verbs (hablar, comer, vivir) first, then apply the same patterns to other regular verbs.

## 🔹 Ser vs Estar - The Two "To Be" Verbs

Spanish has **two verbs** that both mean "to be": **ser** and **estar**. Understanding when to use each one is crucial for Spanish learners.

### Basic Difference

**SER** = What something **IS** (essence, identity, characteristics)  
**ESTAR** = How something **IS** (condition, state, location)

**Key Question:**

- **What** is it? → Use **SER**
- **How** is it? → Use **ESTAR**

### Conjugation

**ser (to be)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | soy         |
| tú               | eres        |
| él/ella/Ud.      | es          |
| nosotros/as      | somos       |
| vosotros/as      | sois        |
| ellos/ellas/Uds. | son         |

**estar (to be)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | estoy       |
| tú               | estás       |
| él/ella/Ud.      | está        |
| nosotros/as      | estamos     |
| vosotros/as      | estáis      |
| ellos/ellas/Uds. | están       |

### When to Use SER

**Remember: SER = Essence, Identity, What something IS**

**🕐 Time, Date, Day**

- **¿Qué hora es?** - What time is it?
- **Son las dos.** - It's two o'clock.
- **Hoy es lunes.** - Today is Monday.
- **Es el cinco de mayo.** - It's May fifth.

**🌍 Origin (Where from)**

- **¿De dónde eres?** - Where are you from?
- **Soy de Colombia.** - I'm from Colombia.

**👔 Occupation & Profession**

- **Soy carpintero.** - I'm a carpenter.
- **Es médico.** - He's a doctor.

**🏁 Nationality**

- **¿Es ella puertorriqueña?** - Is she Puerto Rican?
- **Es guatemalteca.** - She's Guatemalan.

**⛪ Religion & Political Affiliation**

- **Son católicos.** - They're Catholics.
- **Es socialista.** - It's socialist.

**🪵 Material (Made of)**

- **¿De qué es la mesa?** - What's the table made of?
- **Es de madera.** - It's made of wood.

**👥 Possession & Relationships**

- **¿De quién es la pluma?** - Whose pen is it?
- **Es de Emilio.** - It's Emilio's.
- **Es el esposo de Marta.** - He's Martha's husband.

**📍 Events (Where taking place)**

- **La fiesta es en mi casa.** - The party is at my house.
- **La película es en el cine.** - The movie is at the theater.

**💭 Impersonal Expressions**

- **Es importante practicar.** - It's important to practice.
- **Es necesario estudiar.** - It's necessary to study.

**✨ Essential Characteristics (Personality, inherent traits)**

- **Miguel es sincero.** - Mike is sincere.
- **Es muy inteligente.** - He's very intelligent.

**➕ When followed by a NOUN**

- **Juan es médico.** - John is a doctor. (Juan = doctor)

### When to Use ESTAR

**Remember: ESTAR = Condition, State, How something IS**

**📍 Location (Where something is located)**

- **¿Dónde estás?** - Where are you?
- **Estoy en casa.** - I'm at home.
- **Chile está en América del Sur.** - Chile is in South America.

**🌡️ Condition & State (Temporary)**

- **¿Cómo está la sopa?** - How's the soup?
- **La sopa está fría.** - The soup is cold.
- **Estoy muy bien.** - I'm very well.

**🏃 Progressive Tenses (Ongoing actions)**

- **¿Qué estás comiendo?** - What are you eating?
- **Estoy estudiando español.** - I'm studying Spanish.

**🗣️ Common Idiomatic Expressions**

- **estar de acuerdo** - to be in agreement
- **estar de pie** - to be standing
- **estar en camino** - to be on the way
- **estar en las nubes** - to daydream

### SER vs ESTAR with Adjectives

The choice between **ser** and **estar** can **change the meaning** of the same adjective:

| Adjective    | With SER (essence)                | With ESTAR (condition)              |
| ------------ | --------------------------------- | ----------------------------------- |
| **verde**    | Es verde (it's green in color)    | Está verde (it's unripe)            |
| **aburrido** | Es aburrido (he's boring)         | Está aburrido (he's bored)          |
| **listo**    | Es listo (he's smart)             | Está listo (he's ready)             |
| **callado**  | Es callado (he's quiet by nature) | Está callado (he's being quiet now) |

**Examples:**

**La manzana es verde.** - The apple is green (in color).  
**La manzana está verde.** - The apple is green (unripe).

**El profesor es aburrido.** - The professor is boring.  
**El profesor está aburrido.** - The professor is bored.

### Location vs Origin vs Events

This is a common confusion area. Here's how to distinguish:

**Origin (where from)** → **SER**

- **Mónica es de España.** - Monica is from Spain.

**Current Location (where now)** → **ESTAR**

- **Mónica está en Venezuela.** - Monica is in Venezuela.

**Event Location (where taking place)** → **SER**

- **La fiesta es en la casa de Mónica.** - The party is at Monica's house.

### Quick Decision Rules

**Use SER when:**

1. **Noun follows the verb** → Juan **es** médico
2. **Adjective describes essence** → Es muy **inteligente**
3. **Time, date, day** → **Son** las tres
4. **Origin** → **Soy** de México
5. **Events taking place** → La clase **es** en el aula

**Use ESTAR when:**

1. **Location** → **Estoy** en casa
2. **Adjective describes condition** → **Está** enfermo
3. **Progressive tenses** → **Estoy** comiendo
4. **Temporary states** → **Está** cerrado

> **💡 Memory Tip:** Think of **SER** as "**S**erious" (permanent, essential) and **ESTAR** as "**E**verything **T**emporary" (conditions, locations, states that can change).

## 🔹 Stem-Changing Verbs

### What Are Stem-Changing Verbs?

Unlike regular verbs where only the ending changes, **stem-changing verbs** also change their stem when conjugated. The stem is the part of the infinitive that remains after removing -ar, -er, or -ir.

**Example:**

- **hablar** → stem: *habl* + ending: *ar*
- **comer** → stem: *com* + ending: *er*  
- **vivir** → stem: *viv* + ending: *ir*

**Key Rule:** The stem change occurs in **all forms except nosotros and vosotros**.

### Three Types of Stem Changes

**o → ue** (boot verbs)

- The letter **o** in the stem changes to **ue**
- Example: *contar* → *cuento*, *cuentas*, *cuenta*

**e → ie** (boot verbs)

- The letter **e** in the stem changes to **ie**
- Example: *cerrar* → *cierro*, *cierras*, *cierra*

**e → i** (only -ir verbs)

- The letter **e** in the stem changes to **i**
- Example: *repetir* → *repito*, *repites*, *repite*

### Conjugation Pattern: o → ue

**contar (to count, to tell)**

| Subject Pronoun  | Conjugation | Stem Change |
| ---------------- | ----------- | ----------- |
| yo               | cuento      | ✅ o → ue    |
| tú               | cuentas     | ✅ o → ue    |
| él/ella/Ud.      | cuenta      | ✅ o → ue    |
| nosotros/as      | contamos    | ❌ no change |
| vosotros/as      | contáis     | ❌ no change |
| ellos/ellas/Uds. | cuentan     | ✅ o → ue    |

**mover (to move) - er verb example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | muevo       |
| tú               | mueves      |
| él/ella/Ud.      | mueve       |
| nosotros/as      | movemos     |
| vosotros/as      | movéis      |
| ellos/ellas/Uds. | mueven      |

**dormir (to sleep) - ir verb example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | duermo      |
| tú               | duermes     |
| él/ella/Ud.      | duerme      |
| nosotros/as      | dormimos    |
| vosotros/as      | dormís      |
| ellos/ellas/Uds. | duermen     |

### Conjugation Pattern: e → ie

**cerrar (to close)**

| Subject Pronoun  | Conjugation | Stem Change |
| ---------------- | ----------- | ----------- |
| yo               | cierro      | ✅ e → ie    |
| tú               | cierras     | ✅ e → ie    |
| él/ella/Ud.      | cierra      | ✅ e → ie    |
| nosotros/as      | cerramos    | ❌ no change |
| vosotros/as      | cerráis     | ❌ no change |
| ellos/ellas/Uds. | cierran     | ✅ e → ie    |

**entender (to understand) - er verb example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | entiendo    |
| tú               | entiendes   |
| él/ella/Ud.      | entiende    |
| nosotros/as      | entendemos  |
| vosotros/as      | entendéis   |
| ellos/ellas/Uds. | entienden   |

**mentir (to lie) - ir verb example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | miento      |
| tú               | mientes     |
| él/ella/Ud.      | miente      |
| nosotros/as      | mentimos    |
| vosotros/as      | mentís      |
| ellos/ellas/Uds. | mienten     |

### Conjugation Pattern: e → i

**repetir (to repeat)**

| Subject Pronoun  | Conjugation | Stem Change |
| ---------------- | ----------- | ----------- |
| yo               | repito      | ✅ e → i     |
| tú               | repites     | ✅ e → i     |
| él/ella/Ud.      | repite      | ✅ e → i     |
| nosotros/as      | repetimos   | ❌ no change |
| vosotros/as      | repetís     | ❌ no change |
| ellos/ellas/Uds. | repiten     | ✅ e → i     |

**competir (to compete) - another example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | compito     |
| tú               | compites    |
| él/ella/Ud.      | compite     |
| nosotros/as      | competimos  |
| vosotros/as      | competís    |
| ellos/ellas/Uds. | compiten    |

**pedir (to ask for, to order) - another example**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | pido        |
| tú               | pides       |
| él/ella/Ud.      | pide        |
| nosotros/as      | pedimos     |
| vosotros/as      | pedís       |
| ellos/ellas/Uds. | piden       |

### Common o → ue Stem-Changing Verbs

| Verb        | Translation           | Verb     | Translation                |
| ----------- | --------------------- | -------- | -------------------------- |
| almorzar    | to eat lunch          | morder   | to bite                    |
| aprobar     | to approve            | morir    | to die                     |
| colgar      | to hang               | mostrar  | to show                    |
| contar      | to count, to tell     | mover    | to move (an object)        |
| costar      | to cost               | probar   | to prove, test, sample     |
| devolver    | to return (an object) | recordar | to remember                |
| dormir      | to sleep              | resolver | to solve                   |
| encontrar   | to find               | rogar    | to beg, pray               |
| envolver    | to wrap               | sonar    | to sound, ring             |
| soñar (con) | to dream (about)      | tostar   | to toast                   |
| volar       | to fly                | volver   | to return (from someplace) |

### Common e → ie Stem-Changing Verbs

| Verb      | Translation         | Verb        | Translation           |
| --------- | ------------------- | ----------- | --------------------- |
| acertar   | to guess, get right | hervir      | to boil               |
| advertir  | to advise, warn     | mentir      | to lie                |
| cerrar    | to close, shut      | negar       | to deny               |
| comenzar  | to begin            | pensar (en) | to think about        |
| confesar  | to confess          | perder      | to lose               |
| consentir | to consent          | preferir    | to prefer             |
| convertir | to convert          | defender    | to defend             |
| empezar   | to begin            | encender    | to light, kindle      |
| entender  | to understand       | fregar      | to scrub, wash dishes |

### Common e → i Stem-Changing Verbs

| Verb      | Translation      | Verb      | Translation             |
| --------- | ---------------- | --------- | ----------------------- |
| bendecir* | to bless         | medir     | to measure              |
| colegir   | to deduce        | pedir     | to ask for, to order    |
| competir  | to compete       | perseguir | to pursue, to persecute |
| conseguir | to get, obtain   | reír      | to laugh                |
| corregir  | to correct       | repetir   | to repeat               |
| decir*    | to say, tell     | seguir    | to follow, continue     |
| despedir  | to dismiss, fire | servir    | to serve                |
| elegir    | to elect         | sonreír   | to smile                |
| freír     | to fry           | gemir     | to groan, moan          |
| impedir   | to impede        | maldecir* | to curse                |

> **Note:** Verbs marked with * have irregular yo forms: decir (yo digo), bendecir, maldecir.

### Important Notes

**"Boot" Pattern**

- Stem-changing verbs are often called "boot verbs" because when you draw a line around the forms that change, it looks like a boot
- The **nosotros** and **vosotros** forms always keep the original stem

**e → i Changes Only Apply to -ir Verbs**

- This pattern is exclusive to -ir verbs
- -ar and -er verbs never follow the e → i pattern

**Special Irregular Cases**

- **decir** (to say): *yo digo* (not "yo dico")
- **bendecir** and **maldecir** also have irregular yo forms

**Endings Remain Regular**

- Only the stem changes; the endings follow normal patterns
- -ar verbs still use -ar endings, -er verbs use -er endings, etc.

> **💡 Study Tip:** Practice with the "boot" visual - imagine drawing a boot around the forms that change (yo, tú, él/ella/Ud., ellos/ellas/Uds.) while nosotros and vosotros stay outside the boot with their original stems.

## 🔹 Common Irregular Verbs

### Why Learn Irregular Verbs?

While regular and stem-changing verbs follow predictable patterns, **irregular verbs** don't follow standard conjugation rules. These verbs must be **memorized** because their conjugations are unique. However, many irregular verbs are among the most frequently used verbs in Spanish, making them essential to learn.

**Learning Strategy:**

- Create flashcards for each irregular verb group
- Study them frequently (3-4 minutes per session, multiple times daily)
- Focus on memorization rather than trying to find patterns

### Tener and Venir

These two verbs are grouped together because they follow **very similar conjugation patterns**, both ending in **-go** in the yo form and having similar changes throughout.

**tener (to have)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | tengo       |
| tú               | tienes      |
| él/ella/Ud.      | tiene       |
| nosotros/as      | tenemos     |
| vosotros/as      | tenéis      |
| ellos/ellas/Uds. | tienen      |

**venir (to come)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | vengo       |
| tú               | vienes      |
| él/ella/Ud.      | viene       |
| nosotros/as      | venimos     |
| vosotros/as      | venís       |
| ellos/ellas/Uds. | vienen      |

**Pattern Recognition:**

- Both have **-go** endings in the yo form: *tengo, vengo*
- Both change the stem vowel **e → ie** in tú, él/ella/Ud., and ellos/ellas/Uds. forms
- Both keep regular endings in nosotros and vosotros forms

**Examples:**

- **Tengo una casa nueva.** - I have a new house.
- **¿Tienes tiempo?** - Do you have time?
- **Vengo de Colombia.** - I come from Colombia.
- **¿Vienes a la fiesta?** - Are you coming to the party?

### Estar, Ir, and Dar

These three verbs share a common characteristic: they are **irregular in the first person singular (yo form)** but follow more regular patterns in other forms.

**estar (to be) - already covered in detail above**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | estoy       |
| tú               | estás       |
| él/ella/Ud.      | está        |
| nosotros/as      | estamos     |
| vosotros/as      | estáis      |
| ellos/ellas/Uds. | están       |

**ir (to go)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | voy         |
| tú               | vas         |
| él/ella/Ud.      | va          |
| nosotros/as      | vamos       |
| vosotros/as      | vais        |
| ellos/ellas/Uds. | van         |

**dar (to give)**

| Subject Pronoun  | Conjugation |
| ---------------- | ----------- |
| yo               | doy         |
| tú               | das         |
| él/ella/Ud.      | da          |
| nosotros/as      | damos       |
| vosotros/as      | dais        |
| ellos/ellas/Uds. | dan         |

**Pattern Recognition:**

- All three have **irregular yo forms**: *estoy, voy, doy*
- **ir** is completely irregular - it doesn't even resemble its infinitive
- **dar** is unusual because it's an -ar verb but uses -er/-ir endings in some forms

**Usage Examples:**

**Estar:**

- **Estoy en casa.** - I'm at home.
- **¿Cómo estás?** - How are you?

**Ir:**

- **Voy al supermercado.** - I'm going to the supermarket.
- **¿Vas a estudiar?** - Are you going to study?

**Dar:**

- **Te doy mi número.** - I'll give you my number.
- **¿Me das agua?** - Will you give me water?

### Flashcard Method

**For Tener and Venir:**

*Front of card:* **tener (to have) / venir (to come)**

*Back of card:*

tengo / vengo  
tienes / vienes  
tiene / viene  
tenemos / venimos  
tenéis / venís  
tienen / vienen

**For Estar, Ir, and Dar:**

*Front of card:* **estar, ir, dar**

*Back of card:*

estoy – voy – doy  
estás – vas – das  
está – va – da  
estamos – vamos – damos  
estáis – vais – dais  
están – van – dan

### Key Learning Points

**Complete Memorization Required**

- Unlike regular verbs, you cannot predict these conjugations from rules
- Focus on frequency: study these cards multiple times daily
- Short, frequent sessions (3-4 minutes) are more effective than long study periods

**High-Frequency Verbs**

- These irregular verbs are among the most commonly used in Spanish
- **tener** appears in many expressions: *tener hambre* (to be hungry), *tener sed* (to be thirsty)
- **ir** is used to form the immediate future: *voy a estudiar* (I'm going to study)
- **estar** is essential for location and temporary states

**Connection to Previous Concepts**

- **estar** was already covered in the "Ser vs Estar" section - this reinforces its importance
- These irregular verbs work with the same pronoun rules learned with regular verbs
- The same three meanings of present tense apply to irregular verbs

> **💡 Study Strategy:** Carry your verb flashcards with you and review them several times throughout the day. Consistency beats intensity - frequent short reviews will help you memorize these essential irregular patterns more effectively than cramming.

## 🔹 The Special Verb Form "Hay"

### What is "Hay"?

**"Hay"** is a unique verb form that doesn't change regardless of whether you're talking about one thing or many things. It's derived from the verb **haber** (to have/there to be) and serves as the Spanish equivalent of English "there is" and "there are."

**Key Characteristic:**

- **One form for everything**: Unlike English, Spanish uses the same word "hay" for both singular and plural

### Two Meanings of "Hay"

**hay** = there is / there are

**Examples:**

**Singular (there is):**

```markdown
Hay un libro encima de la mesa.
→ There is a book on the table.

Hay un hotel en el centro.
→ There is a hotel downtown.
```

**Plural (there are):**

```markdown
Hay muchos libros en la biblioteca.
→ There are many books in the library.

Hay dos baños en mi casa.
→ There are two bathrooms in my house.

Hay cuatro océanos en el mundo.
→ There are 4 oceans in the world.
```

**Mixed (singular and plural together):**

```markdown
Hay un libro y una pluma en la mesa.
→ There is a book and a pen on the table.
```

### Using "Hay" in Questions

In questions, **"hay"** maintains the same form but takes on interrogative meanings:

**¿Hay...?** = Is there...? / Are there...?

**Question Examples:**

**Singular questions:**

```markdown
¿Hay un hotel en el centro?
→ Is there a hotel downtown?

¿Hay una chica o dos?
→ Is there one girl or two?
```

**Plural questions:**

```markdown
¿Hay algunos libros por aquí?
→ Are there any books around here?

¿Hay muchos estudiantes en la clase?
→ Are there lots of students in the class?

¿Hay cuatro sillas en el cuarto?
→ Are there 4 chairs in the room?
```

### Answering Questions with "Hay"

**"Hay"** can be used in short answers to confirm or deny the existence of something:

**Affirmative Answers:**

```markdown
¿Hay un hotel en el centro?
→ Is there a hotel downtown?

Sí. Sí hay.
→ Yes. Yes there is.
```

**Negative Answers:**

```markdown
¿Hay algunos libros por aquí?
→ Are there any books around here?

No. No hay.
→ No. No there aren't.
```

### Important Grammar Points

**No Plural Agreement**

- Unlike most Spanish verbs, "hay" never changes form

```markdown
❌ Wrong: Hayen muchos libros
✅ Correct: Hay muchos libros
```

**Word Order**

- "Hay" typically comes at the beginning of the sentence
- The noun or noun phrase follows "hay"
- Location/place usually comes at the end

**Pattern**: **Hay** + **[object/thing]** + **[location]**

```markdown
Hay una mesa en la cocina.
→ There is a table in the kitchen.

Hay tres estudiantes en el aula.
→ There are three students in the classroom.
```

### Usage Patterns

**With Numbers:**

```markdown
Hay dos gatos en el jardín.
→ There are two cats in the garden.

Hay cinco libros en la mesa.
→ There are five books on the table.
```

**With Indefinite Articles:**

```markdown
Hay un problema.
→ There is a problem.

Hay una fiesta esta noche.
→ There is a party tonight.
```

**With Adjectives:**

```markdown
Hay muchos estudiantes.
→ There are many students.

Hay algunos libros interesantes.
→ There are some interesting books.
```

**With Location Expressions:**

```markdown
Hay un banco cerca de aquí.
→ There is a bank near here.

Hay flores en el jardín.
→ There are flowers in the garden.
```

### Common Mistakes to Avoid

**Don't Use "Está/Están" for Existence**

```markdown
❌ Wrong: Están muchos libros en la biblioteca
✅ Correct: Hay muchos libros en la biblioteca
```

**Don't Try to Make "Hay" Agree**

```markdown
❌ Wrong: ¿Hayen estudiantes?
✅ Correct: ¿Hay estudiantes?
```

**Remember the Word Order**

```markdown
❌ Less common: En la mesa hay un libro
✅ Better: Hay un libro en la mesa
```

### Flashcard for "Hay"

**Front of card:** **hay**

**Back of card:** there is, there are

### Practice Examples

**Statements:**

- **Hay una computadora en mi cuarto.** - There is a computer in my room.
- **Hay veinte estudiantes en la clase.** - There are twenty students in the class.

**Questions:**

- **¿Hay agua en el refrigerador?** - Is there water in the refrigerator?
- **¿Hay tareas para mañana?** - Is there homework for tomorrow?

**Answers:**

- **Sí, hay.** - Yes, there is/are.
- **No, no hay.** - No, there isn't/aren't.

> **💡 Memory Tip:** Think of "hay" as the Spanish "there is/are" that never changes its form. Whether you're talking about one elephant or a thousand elephants, you always use "hay" - **Hay un elefante** (there is an elephant) and **Hay mil elefantes** (there are a thousand elephants).
